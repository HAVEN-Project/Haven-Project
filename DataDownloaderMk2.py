...
if data is not None and not data.empty:
    ...
    for ticker in tickers:
        ...
        ticker_data = ticker_data.dropna(subset=['date', 'open', 'high', 'low', 'close', 'volume'])

        if last_date:
            ticker_data = ticker_data[ticker_data['date'] > last_date]
        
        available_cols = [col for col in required_cols if col in ticker_data.columns]

        ticker_data = ticker_data[available_cols]

        ticker_data['date'] = pd.to_datetime(ticker_data['date']).dt.strftime('%Y-%m-%d %H:%M:%S')
    ...
    # ticker_dfs.to_sql(TABLE_NAME, conn, if_exists='append', index=False)
